---
title: "Programme summary report"
subtitle: "Impact evaluation findings from (nearly) all projects from the Detect, Protect and Prevent  (DPP) programme"
author: "Craig Parylo"
date: today
...
# project details from grants spreadsheet
df_grants <- readRDS(
  file = here::here('data', 'general', 'df_grants.Rds')
)

# identify updated start dates for the case study projects
df_case_studies_start <- 
  readRDS(
    file = here::here('data', 'project', 'ddp_projects.Rds')
  ) |> 
  dplyr::select(
    project_code, project_id, started_month
  ) |> 
  # exclude projects with known issues
  dplyr::filter(
    !project_id %in% c('P1', 'P13', 'P14')
  )

# get a list of dpp projects
dpp_projects <-
  df_grants |> 
  dplyr::filter(
    # exclude national focussed projects
    !calc_link %in% c('National'),
    # problematic case study grants
    !grant_id %in% c(
      '2023-054', # P1
      '2023-055', # P13
      'pseudo_rachel@_33' # P14
    )
  ) |> 
  # convert start date to a yearmon
  dplyr::mutate(started_month = zoo::as.yearmon(calc_start)) |> 
  # update the start month for the case study projects
  dplyr::left_join(
    y = df_case_studies_start |> 
      dplyr::select(
        grant_id = project_code,
        updated_start_month = started_month
      ) |> 
      dplyr::distinct(),
    by = dplyr::join_by(grant_id == grant_id)
  ) |> 
  dplyr::mutate(
    started_month = dplyr::coalesce(
      updated_start_month,
      started_month
    )
  ) |> 
  dplyr::filter(
    ...
    !is.na(started_month),
    # exclude projects starting before March 2022 (lowest time point)
    started_month >= zoo::as.yearmon("Mar 2022")
  )
...
df_practices <- 
  dpp_practices2 |> 
  dplyr::filter(grant_id %in% df_project$grant_id)

# add in a flag for any practice subject to DPP funding
df_matching <- 
  df_matching |> 
  dplyr::mutate(
    flag_dpp_practice = prac_code %in% dpp_practices2$practice_code
  )
...
# add gp assignment method and nearest reporting time period
df_project <-
  df_project |> 
  # add in gp assignment method
  dplyr::left_join(
    y = dpp_practices2 |> 
  ...
  # add in nearest CVDPREVENT reporting time period as intervention start
  dplyr::left_join(
    y = df_cvd |> 
      dplyr::select(year_month, time_period_id) |> 
      dplyr::distinct(),
    by = dplyr::join_by(dplyr::closest(started_month <= year_month))
  )
...
temp_proj_summary_excluded <-
  df_grants |> 
  # look at grants not in the project summary
  dplyr::filter(
    !grant_id %in% temp_proj_summary$grant_id
  ) |> 
  # determine why they were excluded
  dplyr::mutate(
    exclusion_reason = dplyr::case_when(
      is.na(calc_start) ~ "No start date provided",
      calc_start |> 
        as.Date() |> 
        zoo::as.yearmon() < zoo::as.yearmon("Mar 2022") ~ 
          "Start before March 2022",
      calc_link %in% c('National') | 
        grant_id %in% c('2023-055', 'pseudo_rachel@_33') ~ 
          "National geographic scope",
      grant_id %in% c('2023-054') ~ 
        "Start after last reporting period"
    )
  ) |> 
  dplyr::select(
    grant_id,
...
temp_proj_summary_excluded |> 
  ...
  dplyr::select(-c(calc_start, calc_link)) |> 
  ...
  gt::cols_align('left') |> 
  gt::cols_label(
    grant_id = "ID (or pseudo key)",
    initiative_title = "Title",
    dpp_category = "DPP focus",
...
temp_df_practice_start_id <- df_practices |> 
  ...
  dplyr::left_join(
    y = df_project |> 
      dplyr::select(grant_id, started_time_period = time_period_id) |> 
      dplyr::distinct(grant_id, .keep_all = TRUE),
    by = dplyr::join_by(grant_id == grant_id)
  ) |> 
...
p <- plot_cvd_metric_timeseries_overall_v2(
...
Projects began their interventions at different times with some starting as early as July 2022 and others much later in August 2024. This poses challenges in evaluating impact because there is no single implementation date after which we can assume the outcomes are influenced by the activities.
...
df_temp <-
  ...
  dplyr::left_join(
    ...
    by = dplyr::join_by(time_period_id == time_period_id)
  ) |> 
  dplyr::filter(!is.na(grant_id))
...
df_temp |> 
  ...
  gt::cols_label(
    grant_id = 'Project code',
    initiative_title = 'Title',
    dpp_category = 'DPP category',
    practice_assignment = "GP assignment method",
    practice_count = 'Practices covered'
  ) |> 
  gt::tab_options(quarto.disable_processing = TRUE)
```
...
This means where a GP practice is part of more than one DPP project then the intervention date, which is the date which can consider DPP activities started, is taken to be the earliest date for the relevant projects.
...
ra$month_most_projects <-
  df_temp |> 
  dplyr::summarise(
    project_count = dplyr::n_distinct(grant_id),
    .by = year_month
  ) |> 
...
did::aggte(did, type = 'simple', na.rm = TRUE) |> 
  did::tidy() |> 
  dplyr::select(-dplyr::starts_with(c('term', 'point.'))) |>
  dplyr::mutate(
    # flag periods where the confidence limit does not cover 0
...
did_event <- did::aggte(did, type = 'dynamic', na.rm = TRUE)
did::ggdid(did_event) +
  ggplot2::labs(
...
did::aggte(did, type = 'group', na.rm = TRUE) |> 
  did::tidy() |> 
  dplyr::select(-dplyr::starts_with(c('term', 'point.'))) |>
  dplyr::mutate(
    # flag periods where the confidence limit does not cover 0
...
did::aggte(did, type = 'calendar', na.rm = TRUE) |> 
  did::ggdid()
```
